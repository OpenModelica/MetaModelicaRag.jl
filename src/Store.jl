...
function insert_chunk_row(db::SQLite.DB, chunk, vec::Vector{Float32})
    execute(db,
...
function insert_chunk(db::SQLite.DB, chunk, vec::Vector{Float32})
    SQLite.transaction(db) do
        insert_chunk_row(db, chunk, vec)
    end
end

function insert_chunks_batch(db::SQLite.DB, chunks, vecs, paths, mtimes)
    length(chunks) == length(vecs) == length(paths) == length(mtimes) ||
        throw(ArgumentError("chunks, vecs, paths, and mtimes must have the same length"))

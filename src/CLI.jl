...
function load_config(path::String)::Config
    ...
    backend = get(e, "backend", "github_models")
    allowed_backends = join(sort(collect(VALID_EMBED_BACKENDS)), ", ")
    backend in VALID_EMBED_BACKENDS ||
        throw(ArgumentError("Unknown embeddings.backend \"$backend\". Expected one of: $allowed_backends"))
...
function make_embedder(cfg::Config)
    ...
    elseif cfg.embed_backend == "github_models"
        isempty(cfg.github_token) && error("GitHub token required: set GITHUB_TOKEN in the environment")
        GithubModelsEmbedder(cfg.github_token, cfg.embed_model)
    elseif cfg.embed_backend == "llama"
        LlamaEmbedder(cfg.embed_url)
    else
        throw(ArgumentError("Unsupported embedding backend: $(cfg.embed_backend)"))
    end
end
...
Index MetaModelica source files at `path` into the configured store.
...
function cmd_serve(cfg::Config)
    ...
    @info "MetaModelicaRag MCP server ready  ($n chunks indexed)" stderr = stderr
...
function main(args = ARGS)
    isempty(args) && (println(stderr, "Usage: MetaModelicaRag.main([\"index\"|\"serve\"|\"search\"|\"fuzzy\", ...])"); exit(1))
    ...
    while i <= length(args)
        ...
        elseif args[i] in ("--top-k", "-k") && i + 1 <= length(args)
            top_k_search = parse(Int, args[i+1])
            top_k_fuzzy  = parse(Int, args[i+1])
            i += 2
        else
    ...
    elseif command == "search"
        isempty(query_parts) && (println(stderr, "Usage: MetaModelicaRag.main([\"search\", \"<query>\", ...])"); exit(1))
        cmd_search(cfg, join(query_parts, " "); top_k = top_k_search)
    elseif command == "fuzzy"
        isempty(query_parts) && (println(stderr, "Usage: MetaModelicaRag.main([\"fuzzy\", \"<pattern>\"])"); exit(1))
        cmd_fuzzy(cfg, join(query_parts, " "); top_k = top_k_fuzzy)
    else
